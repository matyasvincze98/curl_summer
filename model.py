...
def cluster_encoder_fn(hiddens, n_y, encoder_type, n_enc, z2_distr_kwargs,
                       is_training=True):
...
def latent_encoder_fn(hiddens, y, n_z,
  ...
  with tf.control_dependencies([tfc.assert_rank(hiddens, 2)]):
    ...
    if y_to_concat_encoder_type == 'mlp':
      ...
      raise ValueError('Unknown y_to_concat_encoder_type {}'.format(
                       y_to_concat_encoder_type))
    ...
    if concat_to_z_encoder_type == 'mlp':
      ...
      raise ValueError('Unknown concat_to_z_encoder_type {}'.format(
                       concat_to_z_encoder_type))
...
def latent_decoder_fn(y, n_z, decoder_type, n_dec, z1_distr_kwargs,
                      is_training=True):
