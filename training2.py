...
def get_data_sources(dataset, dataset_params, dataset_kwargs,
  ...
  Args:
    ...
    dataset_params: None for tf DataSets or dict for own data,
       keys: 'batch_size', 'test_batch_size', 'train_every', 'test_every',
             'crop_dim', 'path', 'offset'
  ...
  if dataset_params is None:  # tf DataSet
    ...
    patches['train_images'] = patches['train_images'].reshape(-1,
                                                              ...
                                                              x_dim,
                                                              1)
    patches['train_images'] = patches['train_images'][::train_every, :crop_dim,
                                                      :crop_dim, :]
    patches['train_labels'] = patches['train_labels'][::train_every]
    ...
    patches['test_images'] = patches['test_images'].reshape(-1, x_dim,
                                                            x_dim, 1)
    patches['test_images'] = patches['test_images'][::test_every, :crop_dim,
                                                    :crop_dim, :]
    patches['test_labels'] = patches['test_labels'][::test_every]
  ...
  return DatasetTuple(train_data, valid_iter, valid_data,
...
def setup_training_and_eval_graphs(x, beta_y, beta_z,
  ...
  x_in = tfc.placeholder(tf.float32, shape=x_shape)
  z1_in = tfc.placeholder(tf.float32,
                          shape=z1_samples_from_z2_prior_samples.shape)
  z2_in = tfc.placeholder(tf.float32, shape=z2_prior_samples.shape)
  ...
  z1_from_x_in_z2_in = curl_model.infer_latent(hiddens=hiddens_from_x_in,
                                               y=z2_in)
  ...
  laplace_distr = curl_model._latent_encoder(hiddens=hiddens_from_x_in,
                                             y=z2_from_x_in.sample())
  laplace_variance = tf.reduce_mean(laplace_distr.variance(), axis=1)
  laplace_constr = laplace_variance - 1
...
def run_training(
  ...
  beta_y = tfc.placeholder(tf.float32, shape=[], name='beta_y')
  beta_z = tfc.placeholder(tf.float32, shape=[], name='beta_z')
  y_train = None
  y_valid = None
  y_test = None
  ...
  if tb_dir is not None:
    tb_writer = tf.summary.FileWriter(tb_dir,
                                      sess.graph if tb_save_graph else None)
  ...
  for step in range(n_steps):
    ...
    if output_type == 'normal' and output_sd is None:
      output_sd_var = next((var for var in
                            model_train._data_decoder.trainable_variables
                            if var.name.find('observ_sd') >= 0),
                           None)
      if output_sd_var is not None:
        logging.info('observ_sd train: %f',
                     output_sd_var.eval(session=sess)[0].item())
    ...
    if (step + 1) % report_interval == 0:
      ...
      if output_type == 'normal' and output_sd is None:
        output_sd_var = next((var for var in
                              model_eval._data_decoder.trainable_variables
                              if var.name.find('observ_sd') >= 0),
                             None)
        if output_sd_var is not None:
          logging.info('observ_sd test: %f',
                       output_sd_var.eval(session=sess)[0].item())
